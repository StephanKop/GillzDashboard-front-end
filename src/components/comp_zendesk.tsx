    ...
    function pushNotification () {
        if (lastArrayItem !== tickets[0].id) {
            ...
            notification.onclick = function (event) {
                ...
                window.open('http://localhost:3000/' + tickets[0].name);
            }
            }, 6000);
        }
    }
                ...
                <tbody className={'zendesk-container__table__body'}>
                    ...
                    {currentPageData.map((ticketData, index) => (
                        <tr key={index} className={'zendesk-container__table__body__data'}>
                            <td style={{maxWidth: props.columnWidth}}>{ticketData.name}</td>
                            ...
                            <td style={{maxWidth: props.columnWidth}} id={'priorityid' + ticketData.id}>{ticketData.priority}</td>
                            {/*<td>{ticketData.status}</td>*/}
                            <td style={{maxWidth: props.columnWidth}}><a target="_blank" href={'https://gillztest.zendesk.com/agent/tickets/' + ticketData.id}><img src={require('../img/external-link.svg')}/></a></td>
                        </tr>
                    ))}
                </tbody>
            </table>
